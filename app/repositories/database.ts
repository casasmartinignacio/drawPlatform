...
class Database {
  ...
  private async readDB(): Promise<Draw[]> {
    try {
  ...
  private async writeDB(data: Draw[]): Promise<void> {
      await fs.writeFile(DB_PATH, JSON.stringify(data, null, 2));
  }

  async findAll(): Promise<Draw[]>{ //json parseado
      return await this.readDB();
  }

  async create(drawing: Omit<Draw, "id">) {
